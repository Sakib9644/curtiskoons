                                ...
                                <table class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        @forelse($healthGoals as $goal)
                                            <tr>
                                                <td>{{ $loop->iteration + ($healthGoals->currentPage() - 1) * $healthGoals->perPage() }}
                                                </td>
                                             <td>{{ $goal->user ? $goal->user->email . ' (' . $goal->user->name . ')' : 'N/A' }}</td>

                                                <td>{{ $goal->goal }}</td>
                                                <td>{!! $goal->methods !!}</td>
                                                <td>{{ $goal->timeline_years }}</td>
                                                <td>
                                                    <a href="{{ route('admin.health_goals.edit', $goal->id) }}"
                                                        class="btn btn-sm btn-warning">Edit</a>
                                                    <form action="{{ route('admin.health_goals.destroy', $goal->id) }}"
