    ...
    <div class="app-sidebar" style="overflow: scroll">
        <div class="side-header">
            <a class="header-brand1" href="{{ route('admin.dashboard') }}">
                <img src="{{ asset(settings()->logo ?? 'default/logo.svg') }}"
                     ...
                     alt="logo"
                     width="{{ settings()->logo_width ?? 100 }}"
                     height="{{ settings()->logo_height ?? 100 }}">
            </a>
        </div>
        ...
        <div class="main-sidemenu">
            ...
            <ul class="side-menu mt-2">
                ...
                <li class="slide">
                    <a class="side-menu__item {{ request()->routeIs('admin.health_goals.*') ? 'has-link active' : '' }}"
                       href="{{ route('admin.health_goals.index') }}">
                        <i class="fa-solid fa-heartbeat side-menu__icon"></i>
                        <span class="side-menu__label">Health Goals</span>
                    </a>
                </li>
                ...
                <!-- User Access Menu -->
                <li class="slide {{ env('ACCESS') === false ? 'd-none' : '' }}">
                    <a class="side-menu__item {{ request()->routeIs('admin.users.*') || request()->routeIs('admin.roles.*') || request()->routeIs('admin.permissions.*') ? 'active' : '' }}"
                    ...
                    <ul class="slide-menu">
                        <li><a href="{{ route('admin.users.index') }}" class="slide-item {{ request()->routeIs('admin.users.*') ? 'active' : '' }}">User</a></li>
                        @if (auth()->user()->hasRole('admin'))
                            <li><a href="{{ route('admin.roles.index') }}" class="slide-item {{ request()->routeIs('admin.roles.*') ? 'active' : '' }}">Role</a></li>
                            <li><a href="{{ route('admin.permissions.index') }}" class="slide-item {{ request()->routeIs('admin.permissions.*') ? 'active' : '' }}">Permission</a></li>
                        @endif
                    </ul>
                </li>
                ...
                <li class="slide">
                    ...
                    <ul class="slide-menu">
                        @foreach ([
                            ...
                            <li>
                                <a href="{{ route('admin.setting.' . $route . '.index') }}"
                                   class="slide-item {{ request()->routeIs('admin.setting.' . $route . '.*') ? 'active' : '' }}">
                                    {{ $label }}
                                </a>
                            </li>
                        @endforeach
